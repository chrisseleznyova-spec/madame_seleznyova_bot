...
async def record_session(user_id: int, username: str, source: str = ""):
    ...
    try:
        ...
        await conn.execute(
            "INSERT INTO sessions_log (user_id, source) VALUES ($1, $2)",
            user_id, source
        )
    finally:
        await conn.close()
...
async def get_stats():
    ...
    try:
        ...
        today_sessions = await conn.fetchval(
            "SELECT COUNT(*) FROM sessions_log WHERE created_date = CURRENT_DATE"
        )
...
SYSTEM_PROMPT = """Ты — ИИ-ассистент Кристины Селезнёвой, коуча ICF. Бот работает как персонаж «Мадам Селезнёва» и помогает людям коротко разобрать их жизненную ситуацию. Твоя задача — через несколько точных вопросов собрать картину происходящего и дать короткий, ясный разбор.
...
Никогда не используй слова: «однозначно», «безусловно», «конечно». Не давай банальных советов.
...
УТОЧНЯЮЩИЕ ВОПРОСЫ:
Задавай 4-6 вопросов. Если после 6 картина ясна — переходи к разбору. Если есть 70-80% понимания — переходи к разбору.
...
async def second_handler(callback: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state(Dialog.mini_sphere)
    await callback.message.answer(
        "Хорошо. Давайте разберём следующую ситуацию.\n\n"
        "Какая сфера жизни сейчас занимает больше всего мыслей?\n"
        "<i>Выберите до двух вариантов, затем нажмите «Готово»</i>",
        reply_markup=sphere_keyboard([])
    )
